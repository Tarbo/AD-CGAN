...
class AD_CGAN():
    ...
    def train(self, sample_interval=50):
        ...
        with open('gen_loss', 'wb') as f:
            pickle.dump(self.generator_loss, f)
        with open('d_loss', 'wb') as f:
            pickle.dump(self.discriminator_loss, f)
        with open('d_acc', 'wb') as f:
            pickle.dump(self.discriminator_acc, f)

    def evaluate_gan(self, epoch, num_samples=300):
        ...
        # create the subplots
        fig = plt.figure(figsize=(30, 25), dpi=90)
        # switch on the axis
        plt.axis('on')
        plt.axis('tight')
        ...
        for idx, perplexity in enumerate(perplexities):
            ...
            ax = sns.scatterplot(x="x-axis", y="y-axis", hue="labels",
        ...
        fig.savefig(img_path+filename, dpi=90, papertype='letter',
    ...
    def load_data(self):
        ...
        # print(data.head())
        # load the supplementary data and concatenate
        anom_samples = np.load('anom.npy')
        # convert to pandas
        anom_samples_df = pd.DataFrame(data=anom_samples, columns=data.columns)
        # concatenate samples
        data = pd.concat([data, anom_samples_df]).sample(frac=1)
        # convert to float 32
        data = data.astype('float32')
        # select normal samples
        X_normal = data[data["label"] == 1]
        num_n = X_normal.shape[0]
        print(f'>>> Number of normal samples: {num_n} ')
        # select anomalous samples
        X_anomaly = data[data["label"] == 2]
        num_a = X_anomaly.shape[0]
        print(f'>>> Number of anomalous samples: {num_a} ')

        # grab the target column
        target = (data.label.values).reshape((data.shape[0], 1))
        # drop the label to get X data
        data.drop(['label'], axis=1, inplace=True)
        # scale the data
        data = scaler.fit_transform(data.values)
        # check to make sure they have the same shape
        print(f'>>> X shape: {data.shape}\tY data: {target.shape} ')
